//
//  AllocatedCheckCollectionCell.m
//  WebsiteCellPractice
//
...
@implementation AllocatedCheckCollectionCell

@synthesize receiptItemsTableView,totalsTableView;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        NSArray *arrayOfViews = [[NSBundle mainBundle] loadNibNamed:@"AllocatedCheckCollectionCell" owner:self options:nil];
...
- (void)createWithBill:(Bill *) billToLoad forReceiptNumber:(int)receiptNumber{
    ...
    self.receiptNumber = receiptNumber-1;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView.tag == 1) {
        return [[self getReceiptOfSelf] getNumberOfItemsInReceipt]+1;
    }
    else {
        return 1;
    }
...
{
    if ([tableView tag] == 1) { //load receiptItems
        if (indexPath.row == [[self getReceiptOfSelf] getNumberOfItemsInReceipt]) {//load addreceiptitemcell
            ...
            [cell setDisplay];
            return cell;
        } else {//load assigned receiptItems
            ...
            ReceiptItem *currentReceiptItem = [[self getReceiptOfSelf] getAssignedReceiptItem:indexPath.row];
            ...
            if (currentReceiptItem.priceValue != 0) {
                cell.priceTextField.text = [NSString stringWithFormat:@"%.02f",currentReceiptItem.splitPriceValue];
            }
        ...
        cell.subTotalTextField.text = [NSString stringWithFormat:@"%.02f",[self getReceiptOfSelf].subTotalValue];
        cell.taxTextField.text = [NSString stringWithFormat:@"%.02f",[self getReceiptOfSelf].taxAmountValue];
        cell.tipTextField.text = [NSString stringWithFormat:@"%.02f",[self getReceiptOfSelf].tipAmountValue];
        cell.totalTextField.text = [NSString stringWithFormat:@"%.02f",[self getReceiptOfSelf].grandTotalValue];
        
        //update the display of the tax and tip button
        [cell updateEditTax:[self getReceiptOfSelf].taxPercentage OrTipButton:[self getReceiptOfSelf].tipPercentage];
...
- (void)tapToAddAnItemWasTapped:(AddReceiptItemCell *)sender{
    if (!self.isInEditingMode) {
        [[self getReceiptOfSelf] addReceiptItem:[[ReceiptItem alloc] init]];
        [self.receiptItemsTableView reloadData];
        [self.receiptItemsTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[[self getReceiptOfSelf] getNumberOfAllocatedItemsInReceipt] inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
        ...
        NSIndexPath *indexPathOfAddedCell = [NSIndexPath indexPathForRow:[[self getReceiptOfSelf] allocatedReceiptItemsArray].count-1 inSection:0];
...
- (ReceiptItem *)getReceiptItemForCell:(ReceiptItemCell *)sender{
    ...
    return [[self getReceiptOfSelf] getAssignedReceiptItem:indexPathOfReceiptItemCell.row];
}
...
- (void)textField:(UITextField *)textField beganEditingIn:(ReceiptItemCell *)receiptItemCellEdited{
    ...
    self.receiptItemsTableView.contentInset = UIEdgeInsetsMake(0, 0, 60, 0);
...
- (Receipt *)getReceiptOfSelf{
    return [self.currentBill getReceipt:self.receiptNumber];
}
